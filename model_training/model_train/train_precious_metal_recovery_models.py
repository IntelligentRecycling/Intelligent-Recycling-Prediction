...
# -*- coding: utf-8 -*-
"""
Precious Metal Recovery Rate Prediction Model Training Script
Training and optimization of precious metal recovery rate prediction models based on machine learning algorithms
"""
...
# Configure logging
logging.basicConfig(filename='train_results/precious_metal_results/model_optimization.log', level=logging.INFO, 
...
def main():
    ...
    df = pd.read_excel('precious_metal_train_dataset.xlsx', engine='openpyxl')
    
    # Separate features X and target Y
    # Features: first 37 columns (CaO to Duration)
    # Targets: last 5 columns (R-Au, R-Ag, R-Pd, R-Pt, R-Rh)
    X = df.iloc[:, :37]
    Y = df.iloc[:, 37:]
    
    # Load test data
    print("Loading test data...")
    df_test = pd.read_excel('precious_metal_test_dataset.xlsx', engine='openpyxl')
    test_X = df_test.iloc[:, :37]
    ...
    predictions_writer = pd.ExcelWriter('train_results/precious_metal_results/predicted_results.xlsx', engine='openpyxl')
    results_writer = pd.ExcelWriter('train_results/precious_metal_results/results.xlsx', engine='openpyxl')
    ...
    try:
        ...
        for metal in Y.columns:
            ...
            for model_name, model in models.items():
                ...
                prefix_model = 'baye'
                bayes_params, bayes_target = bayesian_optimization(
                    model, param_ranges[model_name], X_train, y_train)
                ...
                if model_name == 'XGBoost':
                    model_op_name = f'train_results/precious_metal_results/{model_name}_{metal}_{prefix_model}.json'
                    model.save_model(model_op_name)
                else:
                    import joblib
                    model_op_name = f'train_results/precious_metal_results/{model_name}_{metal}_{prefix_model}.joblib'
            ...
            try:
                predictions_df.to_excel(predictions_writer, sheet_name=f'{metal}_predictions', index=False)
                tmp_df.to_excel(results_writer, sheet_name=f'{metal}_results', index=False)
                print(f"Results saved for {metal}")
            except Exception as e:
                print(f"Warning: Could not save Excel results for {metal}: {e}")
        ...
        try:
            ...
            print(f"Warning: Error closing Excel writers: {e}")
    
    print("\nModel training completed! Results saved to 'train_results/precious_metal_results/' directory.")
